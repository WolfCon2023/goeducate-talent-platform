...
export function CoachPlayerMap() {
  ...
  async function load() {
    ...
    try {
      ...
      const res = await apiFetch<PlayerByStateResponse>("/search/players/by-state", { token });
      setData(res);
    } catch (err) {
  ...
  async function loadPlayersFor(s: PlayerByState | null, page: number) {
    ...
    try {
      ...
      const res = await apiFetch<PlayersInStateResponse>(`/search/players/by-state/${encodeURIComponent(code)}?limit=${playersPageSize}&skip=${skip}`, {
        token,
    ...
    <Card>
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[color:var(--muted)]">Public player profiles by state (click a state to browse).</p>
        </div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[1.6fr_1fr]">
        ...
        <div className="grid gap-4">
          <div className="rounded-2xl border border-[color:var(--border)] bg-[var(--surface)] p-4">
            ...
            <div className="mt-3 text-xs text-[color:var(--muted-2)]">Darker indigo = more players in that state.</div>
          ...
          <div className="rounded-2xl border border-[color:var(--border)] bg-[var(--surface)] p-4">
            ...
            {selected ? (
              <div className="mt-4">
                <div className="text-xs uppercase tracking-wide text-[color:var(--muted-2)]">Players in state</div>

                {playersLoading ? <div className="mt-2 text-sm text-[color:var(--muted)]">Loading…</div> : null}
                  ...
                  <div className="mt-3">
                    ...
                    <div className="mb-2 flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        className="rounded-md border border-white/10 bg-white/5 px-3 py-1 text-sm text-white/90 hover:bg-white/10 disabled:opacity-50"
                        disabled={playersLoading || (playersData.total ?? 0) <= (playersPage + 1) * playersPageSize}
                      ...
                      <table className="w-full text-left text-sm">
                        ...
                        <tbody>
                          {playersData.players.map((p) => {
                            ...
                            const rating = typeof p.latestOverallGrade === "number" && Number.isFinite(p.latestOverallGrade) ? p.latestOverallGrade : null;
                            return (
