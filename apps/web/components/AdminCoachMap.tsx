...
export function AdminCoachMap() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [data, setData] = useState<CoachByStateResponse | null>(null);
  const [selected, setSelected] = useState<CoachByState | null>(null);
  const [tooltip, setTooltip] = useState<Tooltip>(null);

  const [coachesLoading, setCoachesLoading] = useState(false);
  const [coachesError, setCoachesError] = useState<string | null>(null);
  const [coachesData, setCoachesData] = useState<CoachesInStateResponse | null>(null);
  const [coachesPage, setCoachesPage] = useState(0);
  const coachesPageSize = 25;

  const containerRef = useRef<HTMLDivElement | null>(null);
  const coachesAbortRef = useRef<AbortController | null>(null);
  ...
  async function load() {
    ...
    try {
      ...
      const res = await apiFetch<CoachByStateResponse>("/admin/coaches/by-state", { token });
      setData(res);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load coach locations");
    } finally {
      setLoading(false);
    }
  }

  async function loadCoachesFor(s: CoachByState | null, page: number) {
    setCoachesError(null);
    setCoachesData(null);
    coachesAbortRef.current?.abort();
    if (!s) return;

    const maybeCode = String(s.code ?? "").trim();
    const code = maybeCode || US_STATE_NAME_TO_CODE[String(s.name ?? "").trim()] || "";
    if (!code) {
      setCoachesError("Missing state code for selected state.");
      return;
    }

    setCoachesLoading(true);
    const controller = new AbortController();
    coachesAbortRef.current = controller;
    try {
      const token = getAccessToken();
      const role = getTokenRole(token);
      if (!token) throw new Error("Please login first.");
      ...
      const skip = Math.max(0, page) * coachesPageSize;
      const res = await apiFetch<CoachesInStateResponse>(`/admin/coaches/by-state/${encodeURIComponent(code)}?limit=${coachesPageSize}&skip=${skip}`, {
        token,
      ...
      setCoachesError(err instanceof Error ? err.message : "Failed to load coaches for state");
    } finally {
  ...
  useEffect(() => {
    setCoachesPage(0);
    void loadCoachesFor(selected, 0);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selected?.code, selected?.name]);

  useEffect(() => {
    if (!selected) return;
    void loadCoachesFor(selected, coachesPage);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [coachesPage]);

  const byName = useMemo(() => {
    const m = new Map<string, CoachByState>();
  ...
  return (
    <Card>
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold">Coach map</h2>
          <p className="mt-1 text-sm text-[color:var(--muted)]">Best-effort coach locations by state (from institution location/regions).</p>
        </div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[1.6fr_1fr]">
        ...
        <div className="grid gap-4">
          ...
          <div className="rounded-2xl border border-[color:var(--border)] bg-[var(--surface)] p-4">
            ...
            <div className="mt-2 text-sm text-[color:var(--muted)]">
              {selected ? (
                <>
                  ...
                  <div className="mt-1">Coaches: {selected.count}</div>
                </>
              ) : (
            ...
            {selected ? (
              <div className="mt-4">
                <div className="text-xs uppercase tracking-wide text-[color:var(--muted-2)]">Coaches in state</div>

                {coachesLoading ? <div className="mt-2 text-sm text-[color:var(--muted)]">Loading…</div> : null}
                {coachesError ? <div className="mt-2 text-sm text-red-300">{coachesError}</div> : null}

                {coachesData ? (
                  <div className="mt-3">
                    <div className="mb-2 text-xs text-[color:var(--muted-2)]">
                      Showing {coachesData.coaches.length} of {coachesData.total} · Page {coachesPage + 1} of{" "}
                      {Math.max(1, Math.ceil((coachesData.total ?? 0) / coachesPageSize))}
                    </div>
                    <div className="mb-2 flex items-center gap-2">
                      <button
                        ...
                        disabled={coachesLoading || coachesPage <= 0}
                        onClick={() => setCoachesPage((p) => Math.max(0, p - 1))}
                      >
                        Prev
                      </button>
                      <button
                        type="button"
                        className="rounded-md border border-white/10 bg-white/5 px-3 py-1 text-sm text-white/90 hover:bg-white/10 disabled:opacity-50"
                        disabled={coachesLoading || (coachesData.total ?? 0) <= (coachesPage + 1) * coachesPageSize}
                        onClick={() => setCoachesPage((p) => p + 1)}
                      >
                        Next
                      </button>
                    </div>
                      ...
                      <table className="w-full text-left text-sm">
                        ...
                        <tbody>
                          {coachesData.coaches.map((c) => {
                            const name = `${c.firstName ?? ""} ${c.lastName ?? ""}`.trim() || c.user?.email || "—";
                            return (
                              <tr key={c.userId} className="border-t border-white/10">
                                <td className="px-3 py-2 text-[color:var(--foreground)]">
                                  <Link className="text-indigo-300 hover:text-indigo-200 hover:underline" href={`/coaches/${encodeURIComponent(c.userId)}`}>
                                    {name}
                                  </Link>
                                </td>
                                <td className="px-3 py-2">{c.institutionName || "—"}</td>
                                <td className="px-3 py-2">{c.institutionLocation || "—"}</td>
                                <td className="px-3 py-2">
                                  {c.user?.email ? (
                                    <a className="text-indigo-300 hover:underline" href={`mailto:${c.user.email}`}>
                                      {c.user.email}
                                    </a>
                                  ) : (
                                    "—"
