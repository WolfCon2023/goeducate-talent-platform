...
kbRouter.get("/kb/search", maybeAuth, async (req, res, next) => {
  try {
    ...
    const match: any = {};
    if (!admin) match.status = "published";
    if (tag) match.tags = tag;
    if (category) match.category = category;
    if (helpKey) match.helpKeys = helpKey;

    const find: any = { ...match };
...
kbRouter.get("/kb/articles/:slug", maybeAuth, async (req, res, next) => {
  try {
    ...
    if (!slug) return next(new ApiError({ status: 404, code: "NOT_FOUND", message: "Not found" }));
    const admin = isAdmin(req);
    ...
    const related = await KnowledgeBaseArticleModel.find({
      ...
      ...(admin ? {} : { status: "published" }),
      $or: [
        (article as any).helpKeys?.length ? { helpKeys: { $in: (article as any).helpKeys } } : null,
        (article as any).category ? { category: (article as any).category } : null
    ...
    void KnowledgeBaseEventModel.create({
      ...
      userId: req.user?.id ? new mongoose.Types.ObjectId(req.user.id) : undefined,
      slug,
      meta: { status: (article as any).status }
    }).catch(() => {});
    ...
    return res.json({
      article: {
        ...
        helpfulYesCount: (article as any).helpfulYesCount ?? 0,
        helpfulNoCount: (article as any).helpfulNoCount ?? 0
      },
...
kbRouter.get("/kb/categories", maybeAuth, async (req, res, next) => {
  try {
    const admin = isAdmin(req);
    const match: any = { category: { $ne: null } };
    if (!admin) match.status = "published";
...
kbRouter.post("/kb/articles/:slug/feedback", requireAuth, async (req, res, next) => {
  ...
  try {
    ...
    if (!existing) {
      ...
      if (helpful) article.helpfulYesCount += 1;
      else article.helpfulNoCount += 1;
      await article.save();
