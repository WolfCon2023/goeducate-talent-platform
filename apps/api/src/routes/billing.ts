...
billingRouter.get("/billing/status", requireAuth, requireRole([ROLE.COACH]), async (req, res, next) => {
  try {
    ...
    let subscriptionId: string | null = coach.stripeSubscriptionId ?? null;
    ...
    if (coach.stripeCustomerId) {
      try {
        ...
        if (active) {
          ...
          const renewalTs = typeof (active as any).current_period_end === "number" ? ((active as any).current_period_end as number) : null;
          ...
          try {
            const renewalTs = typeof (active as any).current_period_end === "number" ? ((active as any).current_period_end as number) : null;
            const scheduleId =
              (active as any).schedule && typeof (active as any).schedule === "string"
                ? ((active as any).schedule as string)
                : (active as any).schedule?.id ?? null;
            if (scheduleId && renewalTs) {
              const schedule = await stripe.subscriptionSchedules.retrieve(scheduleId);
              const scheduledPhase = schedule.phases?.find((p: any) => p.start_date === renewalTs);
...
billingRouter.post("/billing/downgrade-monthly", requireAuth, requireRole([ROLE.COACH]), async (req, res, next) => {
  try {
    ...
    const isAnnual =
      (currentPriceId && currentPriceId === env.STRIPE_PRICE_ID_ANNUAL) || (!currentPriceId && currentInterval === "year") || currentInterval === "year";
    const isMonthly =
      (currentPriceId && currentPriceId === env.STRIPE_PRICE_ID_MONTHLY) || (!currentPriceId && currentInterval === "month") || currentInterval === "month";
    ...
    const currentPhaseStart =
      typeof (active as any).current_period_start === "number"
        ? ((active as any).current_period_start as number)
