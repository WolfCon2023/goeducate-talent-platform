...
searchRouter.get("/search/players", requireAuth, requireRole([ROLE.COACH, ROLE.ADMIN, ROLE.EVALUATOR]), async (req, res, next) => {
  try {
    const { q, sport, position, state, gradYearMin, gradYearMax, limit } = req.query as Record<string, string | undefined>;
    const filter: Record<string, any> = { isProfilePublic: true };
