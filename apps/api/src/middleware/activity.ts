...
export async function trackDailyActive(req: Request, _res: Response, next: NextFunction) {
  try {
    if (!req.user?.id) return next();
    // Avoid noise from health checks/root.
    const p = String(req.path ?? "");
    if (p === "/" || p.startsWith("/health") || p.startsWith("/webhooks/")) return next();

    if (!mongoose.isValidObjectId(req.user.id)) return next();
    const now = new Date();
    const day = dayKeyUtc(now);
    const userId = new mongoose.Types.ObjectId(req.user.id);

    // Upsert once per user/day; keep firstSeenAt stable and bump lastSeenAt.
    void DailyActiveUserModel.updateOne(
      { userId, day },
      {
