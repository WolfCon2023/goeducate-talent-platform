...
export async function sendAccessRequestAdminAlert(input: {
  to: string;
  request: { fullName: string; email: string; requestedRole: string; sport: string; sportOther?: string };
}) {
  ...
  return { messageId: (info as any)?.messageId as string | undefined, subject };
}
...
export async function sendAccessRequestApprovedEmail(input: {
  ...
  const html = `
    <div style="font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif; line-height: 1.5;">
      ...
      <p style="margin:0 0 6px 0;">Or paste this invite code:</p>
...
export async function sendAccessRequestRejectedEmail(input: { to: string }) {
  const { env, transporter } = createTransporterOrThrow();
