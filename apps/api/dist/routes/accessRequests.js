...
import { isAccessRequestEmailConfigured, sendAccessRequestAdminAlert, sendAccessRequestApprovedEmail, sendAccessRequestRejectedEmail } from "../email/accessRequests.js";
...
async function createInviteForEmail(opts) {
    ...
    try {
        ...
        if (!adminEmails.length) {
            ...
            await EmailAuditLogModel.create({
                ...
                relatedAccessRequestId: new mongoose.Types.ObjectId(String(created._id)),
                error: { reason: "no_admins" }
            });
        }
        else if (!isAccessRequestEmailConfigured()) {
            await EmailAuditLogModel.insertMany(adminEmails.map((to) => ({
                type: EMAIL_AUDIT_TYPE.ACCESS_REQUEST_ADMIN_ALERT,
                status: EMAIL_AUDIT_STATUS.SKIPPED,
                to,
                subject: "GoEducate Talent – New access request",
        ...
        return res.status(201).json({ accessRequest: { id: String(created._id), status: created.status } });
    }
    catch (err) {
...
accessRequestsRouter.get("/admin/access-requests", requireAuth, requireRole([ROLE.ADMIN]), async (req, res, next) => {
    try {
        ...
        const filter = {};
        if (status && [ACCESS_REQUEST_STATUS.PENDING, ACCESS_REQUEST_STATUS.APPROVED, ACCESS_REQUEST_STATUS.REJECTED].includes(status)) {
...
accessRequestsRouter.get("/admin/access-requests/:id", requireAuth, requireRole([ROLE.ADMIN]), async (req, res, next) => {
    try {
        ...
        if (!item)
            return next(new ApiError({ status: 404, code: "NOT_FOUND", message: "Access request not found" }));
