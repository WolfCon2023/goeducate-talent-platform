...
notificationsRouter.get("/notifications/stream", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
...
notificationsRouter.get("/notifications/me", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
...
notificationsRouter.get("/notifications/unread-count", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
        const userId = new mongoose.Types.ObjectId(req.user.id);
...
notificationsRouter.patch("/notifications/:id/read", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
        ...
        const userId = new mongoose.Types.ObjectId(req.user.id);
        const updated = await NotificationModel.findOneAndUpdate({ _id, userId, readAt: { $exists: false } }, { $set: { readAt: new Date() } }, { new: true }).lean();
        if (!updated)
            return next(new ApiError({ status: 404, code: "NOT_FOUND", message: "Notification not found" }));
...
notificationsRouter.delete("/notifications/:id", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
        if (!mongoose.isValidObjectId(req.params.id)) {
            return next(new ApiError({ status: 404, code: "NOT_FOUND", message: "Notification not found" }));
        }
...
notificationsRouter.delete("/notifications/me", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
        const userId = new mongoose.Types.ObjectId(req.user.id);
        const unreadOnly = String(req.query.unreadOnly ?? "").trim() === "1";
        const query = { userId };
...
notificationsRouter.patch("/notifications/me/read", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
        const userId = new mongoose.Types.ObjectId(req.user.id);
        const unreadOnly = String(req.query.unreadOnly ?? "").trim() === "1";
        const query = { userId };
