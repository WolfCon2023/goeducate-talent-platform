...
profilePhotosRouter.post("/users/me/profile-photo", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH]), upload.single("file"), async (req, res, next) => {
    try {
        const file = req.file;
        if (!file)
            return next(new ApiError({ status: 400, code: "BAD_REQUEST", message: "Missing file" }));
...
//# sourceMappingURL=profilePhotos.js.map