...
authRouter.get("/auth/recovery-questions/me", requireAuth, async (req, res, next) => {
    try {
        ...
        const qs = (user.recoveryQuestions ?? []);
        return res.json({
...
authRouter.put("/auth/recovery-questions/me", requireAuth, authSecQLimiter, async (req, res, next) => {
    ...
    try {
        ...
        const nextQs = await Promise.all(parsed.data.questions.map(async (q) => ({
            questionId: q.questionId,
            question: q.question,
            answerHash: await bcrypt.hash(normalizeAnswer(q.answer), 12)
        })));
        user.recoveryQuestions = nextQs;
        await user.save();
...
async function verifySecurityAnswers(user, answers) {
    const stored = (user.recoveryQuestions ?? []);
    if (!stored.length || stored.length !== answers.length)
        return false;
