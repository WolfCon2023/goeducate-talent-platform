...
function addDaysUtc(day, deltaDays) {
...
async function getConfig(reqUserId) {
    const cfg = (await AdminMetricsConfigModel.findOne({ key: "default" }).lean()) ??
...
adminMetricsRouter.get("/admin/metrics/summary", async (req, res, next) => {
    try {
        ...
        const totalsByRole = Object.fromEntries(totalsByRoleAgg.map((r) => [String(r._id), Number(r.count)]));
        const newByRoleAgg = await UserModel.aggregate([
            { $match: { createdAt: { $gte: start } } },
            { $group: { _id: "$role", count: { $sum: 1 } } }
        ]);
        const newUsersByRole = Object.fromEntries(newByRoleAgg.map((r) => [String(r._id), Number(r.count)]));
        ...
        const dauByRole = Object.fromEntries(dauAgg.map((r) => [String(r._id), Number(r.count)]));
        const wauByRole = Object.fromEntries(wauAgg.map((r) => [String(r._id), Number(r.count)]));
        const mauByRole = Object.fromEntries(mauAgg.map((r) => [String(r._id), Number(r.count)]));
        ...
        const playerPublicCount = playerProfiles.filter((p) => Boolean(p.isProfilePublic)).length;
        const playerProfileCount = playerProfiles.length;
        function summarizeCompletion(rows, compute) {
        ...
        const submissionsByStatus = Object.fromEntries(submissionCountsRaw.map((r) => [String(r._id), Number(r.count)]));
        ...
        const backlog = await FilmSubmissionModel.countDocuments({ status: { $in: openStatuses } });
        const overdueHours = Number(cfg?.overdueHours ?? 72);
        const overdueBefore = new Date(Date.now() - overdueHours * 60 * 60 * 1000);
        const overdue = await FilmSubmissionModel.countDocuments({ status: { $in: openStatuses }, createdAt: { $lt: overdueBefore } });
        ...
        const diffsHours = turnaroundRows
        ...
        const turnaround = {
            ...
            medianHours: percentile(diffsHours, 0.5) != null ? Math.round(percentile(diffsHours, 0.5) * 10) / 10 : null,
            p90Hours: percentile(diffsHours, 0.9) != null ? Math.round(percentile(diffsHours, 0.9) * 10) / 10 : null,
        ...
        const throughputEvaluatorIds = throughputByEvaluator.map((r) => r._id).filter(Boolean);
        const throughputUsers = await UserModel.find({ _id: { $in: throughputEvaluatorIds } })
            .select({ _id: 1, email: 1, firstName: 1, lastName: 1 })
            .lean();
        const throughputUserById = new Map(throughputUsers.map((u) => [String(u._id), u]));
        // Engagement
        ...
        let stripe = null;
        let stripeKpis = { configured: stripeConfigured, mrrCents: null, arrCents: null, active: null, monthly: null, annual: null };
        if (stripeConfigured) {
            try {
                ...
                const relevant = subs.filter((s) => (s.items?.data ?? []).some((it) => it.price?.id === monthlyPriceId || it.price?.id === annualPriceId));
                const monthly = relevant.filter((s) => (s.items?.data ?? []).some((it) => it.price?.id === monthlyPriceId)).length;
                const annual = relevant.filter((s) => (s.items?.data ?? []).some((it) => it.price?.id === annualPriceId)).length;
                const mrr = relevant.reduce((sum, s) => {
                    const items = s.items?.data ?? [];
                    for (const it of items) {
                        const price = it.price;
                        if (!price?.recurring?.interval || typeof price.unit_amount !== "number")
                            continue;
                        if (price.id === monthlyPriceId && price.recurring.interval === "month")
                            sum += price.unit_amount;
                        if (price.id === annualPriceId && price.recurring.interval === "year")
                            sum += Math.round(price.unit_amount / 12);
                    }
                    return sum;
                }, 0);
        ...
        const kbTopViewed = kbViewsAgg.map((r) => ({ slug: String(r._id ?? ""), views: Number(r.views) || 0 }));
        ...
        const kbFeedbackYes = kbFeedbackAgg.find((r) => r._id === true)?.count ?? 0;
        const kbFeedbackNo = kbFeedbackAgg.find((r) => r._id === false)?.count ?? 0;
        ...
        const workloadAgg = await FilmSubmissionModel.aggregate([
            { $match: { status: { $in: openStatuses }, assignedEvaluatorUserId: { $ne: null } } },
            {
                $addFields: {
        ...
        const evaluatorIds = workloadAgg.map((w) => w._id).filter(Boolean);
        const evaluatorUsers = await UserModel.find({ _id: { $in: evaluatorIds } }).select({ _id: 1, email: 1, firstName: 1, lastName: 1 }).lean();
        const evalById = new Map(evaluatorUsers.map((u) => [String(u._id), u]));
        const throughputMap = new Map(throughputByEvaluator.map((r) => [String(r._id), r]));
        const evaluatorWorkload = workloadAgg.map((w) => {
        ...
        void AdminMetricsSnapshotModel.updateOne({ day: todayKey }, {
            $set: {
                day: todayKey,
        ...
        return res.json({
            ...
            evaluations: {
                ...
                turnaround,
                evaluatorThroughput: throughputByEvaluator.map((r) => {
            ...
            revenue: {
                stripe: stripeKpis,
                showcases: showcasePaidAgg.map((r) => ({
            ...
            reliability: {
                ...
                kb: {
                    ...
                    mostNotHelpful: kbMostNotHelpful.map((a) => ({
                        slug: a.slug,
                        title: a.title,
...
adminMetricsRouter.get("/admin/metrics/trends", async (req, res, next) => {
    try {
        ...
        for (const r of rows) {
            ...
            if (!buckets.has(wk)) {
                buckets.set(wk, { week: wk, count: 0, sums: { submissionsNew: 0, evaluationsCompletedNew: 0, coachSearchEvents: 0 }, last: {}, emailFailRatePct: [], coachConversionRatePct: [] });
            }
            ...
            if (typeof r.emailFailRatePct === "number")
                b.emailFailRatePct.push(r.emailFailRatePct);
            if (typeof r.coachConversionRatePct === "number")
                b.coachConversionRatePct.push(r.coachConversionRatePct);
        }
        ...
        const series = {
            ...
            emailFailRatePct: weeksSorted.map((b) => {
                if (!b.emailFailRatePct.length)
                    return null;
                const avg = b.emailFailRatePct.reduce((x, y) => x + y, 0) / b.emailFailRatePct.length;
                return Math.round(avg * 10) / 10;
            }),
            coachConversionRatePct: weeksSorted.map((b) => {
                if (!b.coachConversionRatePct.length)
                    return null;
                const avg = b.coachConversionRatePct.reduce((x, y) => x + y, 0) / b.coachConversionRatePct.length;
                return Math.round(avg * 10) / 10;
            })
        };
