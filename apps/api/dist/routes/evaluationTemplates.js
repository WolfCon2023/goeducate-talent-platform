...
evaluationTemplatesRouter.get("/evaluation-templates", requireAuth, requireRole([ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
...
evaluationTemplatesRouter.get("/evaluation-templates/recommend", requireAuth, requireRole([ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
        ...
        const effectivePosition = sport === "other" || positionRaw === "Other" ? positionOtherRaw || "any" : positionRaw || "any";
        const queries = [
...
evaluationTemplatesRouter.patch("/admin/evaluation-templates/:id", requireAuth, requireRole([ROLE.ADMIN]), async (req, res, next) => {
    ...
    if (!parsed.success)
        return next(zodToBadRequest(parsed.error.flatten()));
    try {
        const updated = await EvaluationTemplateModel.findByIdAndUpdate(req.params.id, {
            ...parsed.data,
        ...
        if (!updated)
            return next(new ApiError({ status: 404, code: "NOT_FOUND", message: "Template not found" }));
...
evaluationTemplatesRouter.delete("/admin/evaluation-templates/:id", requireAuth, requireRole([ROLE.ADMIN]), async (req, res, next) => {
    if (!mongoose.isValidObjectId(req.params.id)) {
        return next(new ApiError({ status: 400, code: "BAD_REQUEST", message: "Invalid id" }));
    }
    try {
...
//# sourceMappingURL=evaluationTemplates.js.map