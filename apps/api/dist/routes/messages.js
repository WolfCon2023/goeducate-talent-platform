...
messagesRouter.get("/messages/recipients", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
        ...
        const roleFilter = role && [ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR].includes(role) ? role : null;
        const userFind = { role: { $in: allowedRecipientRoles } };
        if (roleFilter)
            userFind.role = roleFilter;
        if (rx)
            userFind.$or = [{ email: rx }, { firstName: rx }, { lastName: rx }];
        ...
        const playerProfiles = !roleFilter || roleFilter === ROLE.PLAYER
            ? await PlayerProfileModel.find(rx ? { $or: [{ firstName: rx }, { lastName: rx }, { sport: rx }] } : {}, { userId: 1, firstName: 1, lastName: 1, sport: 1 })
                .limit(limit)
                .lean()
            : [];
        const coachProfiles = !roleFilter || roleFilter === ROLE.COACH
            ? await CoachProfileModel.find(rx ? { $or: [{ firstName: rx }, { lastName: rx }, { institutionName: rx }] } : {}, { userId: 1, firstName: 1, lastName: 1, institutionName: 1, title: 1 })
                .limit(limit)
                .lean()
            : [];
        const evaluatorProfiles = !roleFilter || roleFilter === ROLE.EVALUATOR
            ? await EvaluatorProfileModel.find(rx ? { $or: [{ firstName: rx }, { lastName: rx }, { location: rx }] } : {}, { userId: 1, firstName: 1, lastName: 1, title: 1, location: 1 })
                .limit(limit)
                .lean()
            : [];
        ...
        for (const c of coachProfiles)
            ids.add(String(c.userId));
        for (const e of evaluatorProfiles)
            ids.add(String(e.userId));
        const users = await UserModel.find({
            _id: { $in: Array.from(ids) },
            role: roleFilter ? roleFilter : { $in: allowedRecipientRoles }
        })
            .select({ email: 1, role: 1, firstName: 1, lastName: 1 })
            .limit(50)
            .lean();
        const userById = new Map(users.map((u) => [String(u._id), u]));
        const playerById = new Map(playerProfiles.map((p) => [String(p.userId), p]));
        const coachById = new Map(coachProfiles.map((c) => [String(c.userId), c]));
        const evaluatorById = new Map(evaluatorProfiles.map((e) => [String(e.userId), e]));
...
messagesRouter.post("/messages/conversations", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    ...
    try {
        ...
        if (!recipientUser)
            return next(new ApiError({ status: 404, code: "NOT_FOUND", message: "Recipient not found" }));
        if (String(recipientUser.role) === ROLE.ADMIN) {
...
messagesRouter.post("/messages/conversations/:id/accept", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
        const userId = ensureObjectId(req.user.id);
...
messagesRouter.post("/messages/conversations/:id/decline", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
        const userId = ensureObjectId(req.user.id);
        const convId = ensureObjectId(req.params.id);
        const conv = await ConversationModel.findById(convId);
...
messagesRouter.get("/messages/conversations/:id", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.ADMIN, ROLE.EVALUATOR]), async (req, res, next) => {
    try {
        ...
        const senderIds = Array.from(new Set(messages.map((m) => String(m.senderUserId)).filter(Boolean)));
        ...
        return res.json({
            ...
            messages: messages.reverse().map((m) => ({
                ...
                sender: (() => {
                    ...
                    return { id: String(u._id), email: u.email, role: u.role, displayName };
                })(),
                body: m.body,
...
messagesRouter.post("/messages/conversations/:id/messages", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
