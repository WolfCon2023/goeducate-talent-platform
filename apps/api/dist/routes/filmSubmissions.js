            ...
            void (async () => {
                try {
                    ...
                    if (uniqueInternalIds.length > 0) {
                        await NotificationModel.insertMany(uniqueInternalIds.map((id) => ({
                        ...
                        for (const id of uniqueInternalIds)
                            publishNotificationsChanged(id);
                    }
                    ...
                    if (recipients.length === 0)
                        return;
                    await Promise.all(recipients.map((to) => sendNotificationEmail({
                        to,
...
filmSubmissionsRouter.get("/film-submissions/me", requireAuth, requireRole([ROLE.PLAYER]), async (req, res, next) => {
    try {
        const userId = new mongoose.Types.ObjectId(req.user.id);
...
filmSubmissionsRouter.get("/film-submissions/player/:userId", requireAuth, requireRole([ROLE.COACH, ROLE.ADMIN, ROLE.EVALUATOR]), async (req, res, next) => {
    try {
...
filmSubmissionsRouter.get("/film-submissions/queue", requireAuth, requireRole([ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
...
filmSubmissionsRouter.patch("/film-submissions/:id/status", requireAuth, requireRole([ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    ...
    try {
        ...
        if (film.status === FILM_SUBMISSION_STATUS.NEEDS_CHANGES) {
            ...
            const to = String(user?.email ?? profile?.contactEmail ?? "").trim().toLowerCase();
...
filmSubmissionsRouter.get("/film-submissions/:id", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.ADMIN, ROLE.EVALUATOR]), async (req, res, next) => {
    try {
        if (!mongoose.isValidObjectId(req.params.id)) {
            return next(new ApiError({ status: 404, code: "NOT_FOUND", message: "Film submission not found" }));
        }
...
filmSubmissionsRouter.delete("/film-submissions/:id", requireAuth, requireRole([ROLE.PLAYER]), async (req, res, next) => {
    try {
        if (!mongoose.isValidObjectId(req.params.id)) {
            return next(new ApiError({ status: 404, code: "NOT_FOUND", message: "Film submission not found" }));
        }
...
//# sourceMappingURL=filmSubmissions.js.map