...
function guessStateFromCoachLocation(input) {
...
const AdminUserUpdateSchema = z.object({
    ...
    email: z.preprocess((v) => (typeof v === "string" ? v.trim().toLowerCase() : v), z.string().email().max(254)).optional(),
    username: z
        .preprocess((v) => (typeof v === "string" ? v.trim().toLowerCase() : v), z.string().min(3).max(40))
        .optional(),
    firstName: z.preprocess((v) => (typeof v === "string" ? v.trim() : v), z.string().min(1).max(60)).optional(),
    lastName: z.preprocess((v) => (typeof v === "string" ? v.trim() : v), z.string().min(1).max(60)).optional(),
...
adminRouter.patch("/admin/users/:id", requireAuth, requireRole([ROLE.ADMIN]), async (req, res, next) => {
    ...
    try {
        ...
        if (nextRole === ROLE.EVALUATOR) {
            await EvaluatorProfileModel.updateOne({ userId: _id }, { $set: { firstName: user.firstName, lastName: user.lastName }, $setOnInsert: { userId: _id, isProfilePublic: true } }, { upsert: true });
        }
        const after = {
            email: user.email,
            username: user.username,
