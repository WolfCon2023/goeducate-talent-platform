...
async function createRegistrationFromCheckoutSession(opts) {
    ...
    if (!showcase)
        return { created: false, reason: "showcase_not_found" };
    ...
    const refundPolicyAcceptedAt = session?.metadata?.refundPolicyAcceptedAt
        ? new Date(String(session.metadata.refundPolicyAcceptedAt))
        : undefined;
...
function computeRegistrationState(s, now = new Date()) {
    const status = String(s.status ?? "");
    if (status !== SHOWCASE_STATUS.PUBLISHED)
...
function toPublicShowcase(s) {
    ...
    const isPlaceholderRefund = !rawRefund ||
...
showcasesRouter.get("/showcases", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (_req, res, next) => {
    try {
...
showcasesRouter.get("/showcase-registrations/me", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
        const userId = new mongoose.Types.ObjectId(req.user.id);
        const u = await UserModel.findById(userId).lean();
        const email = String(u?.email ?? "").trim().toLowerCase();
        const match = { $or: [{ userId }] };
        if (email)
            match.$or.push({ email });
        ...
        return res.json({ results: rows.map((r) => ({ ...r, id: String(r._id) })) });
    }
    catch (err) {
        return next(err);
    }
...
// This is a safety net in case Stripe webhooks are delayed/misconfigured.
showcasesRouter.post("/showcase-registrations/confirm", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
        ...
        if (!sessionId)
            return next(new ApiError({ status: 400, code: "BAD_REQUEST", message: "sessionId is required" }));
        const stripe = getStripe(env);
        const session = await stripe.checkout.sessions.retrieve(sessionId);
        // For one-time payments, payment_status is the most useful.
        if (session.payment_status !== "paid") {
            return next(new ApiError({ status: 409, code: "NOT_PAID", message: "Checkout session is not paid" }));
        }
        const result = await createRegistrationFromCheckoutSession({ env, session });
...
showcasesRouter.get("/admin/showcase-registrations", requireAuth, requireRole([ROLE.ADMIN]), async (req, res, next) => {
    try {
...
showcasesRouter.get("/showcases/:idOrSlug", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
...
showcasesRouter.post("/showcases/:idOrSlug/register", requireAuth, requireRole([ROLE.PLAYER, ROLE.COACH, ROLE.EVALUATOR, ROLE.ADMIN]), async (req, res, next) => {
    try {
        ...
        if (userId && mongoose.isValidObjectId(userId)) {
            const u = await UserModel.findById(userId).lean();
            if (u?.email)
                customerEmail = String(u.email).toLowerCase();
            if (u?.role)
                role = u.role;
        }
        const stripe = getStripe(env);
...
showcasesRouter.get("/admin/showcases", requireAuth, requireRole([ROLE.ADMIN]), async (req, res, next) => {
    try {
        ...
        if (status)
            q.status = status;
        if (sport)
            q.sportCategories = sport;
        if (from || to) {
            q.startDateTime = {};
            if (from)
                q.startDateTime.$gte = new Date(from);
            if (to)
                q.startDateTime.$lte = new Date(to);
        }
...
showcasesRouter.put("/admin/showcases/:id", requireAuth, requireRole([ROLE.ADMIN]), async (req, res, next) => {
    ...
    try {
        ...
        if (parsed.data.startDateTime)
            update.startDateTime = new Date(parsed.data.startDateTime);
        if (parsed.data.endDateTime)
            update.endDateTime = new Date(parsed.data.endDateTime);
        if (parsed.data.registrationOpenAt)
            update.registrationOpenAt = new Date(parsed.data.registrationOpenAt);
        if (parsed.data.registrationCloseAt)
            update.registrationCloseAt = new Date(parsed.data.registrationCloseAt);
...
showcasesRouter.delete("/admin/showcases/:id", requireAuth, requireRole([ROLE.ADMIN]), async (req, res, next) => {
    try {
        ...
        if (!mongoose.isValidObjectId(id))
            return next(new ApiError({ status: 400, code: "BAD_REQUEST", message: "Invalid id" }));
        const updated = await ShowcaseModel.findByIdAndUpdate(new mongoose.Types.ObjectId(id), { $set: { status: SHOWCASE_STATUS.ARCHIVED, registrationOpen: false } }, { new: true }).lean();
        if (!updated)
            return next(new ApiError({ status: 404, code: "NOT_FOUND", message: "Showcase not found" }));
        void logAdminAction({
            req,
