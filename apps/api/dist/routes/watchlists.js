...
watchlistsRouter.post("/watchlists/:playerUserId", requireAuth, requireRole([ROLE.COACH, ROLE.ADMIN]), async (req, res, next) => {
    try {
...
watchlistsRouter.delete("/watchlists/:playerUserId", requireAuth, requireRole([ROLE.COACH, ROLE.ADMIN]), async (req, res, next) => {
    try {
        if (!mongoose.isValidObjectId(req.params.playerUserId)) {
            return next(new ApiError({ status: 400, code: "BAD_REQUEST", message: "Invalid playerUserId" }));
        }
