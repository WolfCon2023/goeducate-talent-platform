...
export const ShowcaseCreateSchema = ShowcaseBaseSchema.superRefine((val, ctx) => {
    ...
    if (val.status === "published") {
        const requiredWhenPublished = ["locationName", "addressLine1", "city", "state"];
        for (const k of requiredWhenPublished) {
            if (!String(val[k] ?? "").trim()) {
                ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Required when published.", path: [k] });
            }
        }
    }
});
